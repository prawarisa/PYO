...
@Component({
  selector: 'app-cart-cleaning',
  templateUrl: './cart-cleaning.component.html',
  styleUrls: ['./cart-cleaning.component.scss']
})
export class CartCleaningComponent {
  division: string = '';
  factory: string = '';
  process: string = '';
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        Swal.fire('Success!', 'Doc no. has been created.', 'success').then(() => {
          ...
          this.router.navigate(['/pages/list-request-cleaning']); // ใส่ path ที่ต้องการนำทาง
        });
      } else {
