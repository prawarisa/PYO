...
@Component({
  selector: 'app-cart-oil',
  templateUrl: './cart-oil.component.html',
  styleUrls: ['./cart-oil.component.scss']
})
export class CartOilComponent {
  division: string = '';
  factory: string = '';
  process: string = '';
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        Swal.fire('Success!', 'Doc no. has been created.', 'success').then(() => {
          ...
          this.router.navigate(['/pages/list-request-oil']); // ใส่ path ที่ต้องการนำทาง
        });
      } else {
