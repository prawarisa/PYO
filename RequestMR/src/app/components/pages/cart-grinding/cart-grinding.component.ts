...
@Component({
  selector: 'app-cart-grinding',
  templateUrl: './cart-grinding.component.html',
  styleUrls: ['./cart-grinding.component.scss']
})
export class CartGrindingComponent {
  division: string = '';
  factory: string = '';
  process: string = '';
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        Swal.fire('Success!', 'Doc no. has been created.', 'success').then(() => {
          ...
          this.router.navigate(['/pages/list-request-grinding']); // ใส่ path ที่ต้องการนำทาง
        });
      } else {
